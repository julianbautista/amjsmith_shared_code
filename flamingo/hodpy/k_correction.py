...
class GAMA_KCorrection(KCorrection):
    ...
    """
    def __init__(self, cosmology, k_corr_file=lookup.kcorr_file_rband, 
                 z0=0.1, cubic_interpolation=False, interpolation=True):
        ...
        if not interpolation:
            self.__A_interpolator = self.__initialize_parameter_interpolator_no_interp(A, cmin)
            self.__B_interpolator = self.__initialize_parameter_interpolator_no_interp(B, cmin)
            self.__C_interpolator = self.__initialize_parameter_interpolator_no_interp(C, cmin)
            self.__D_interpolator = self.__initialize_parameter_interpolator_no_interp(D, cmin)
        else:
    ...
    def __initialize_parameter_interpolator_no_interp(self, param, min_col):
        # interpolated polynomial coefficient as a function of colour
        return interp1d(min_col, param, kind='previous', fill_value='extrapolate')
